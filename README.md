...
npm run dev
🗺️ Future Roadmap
1, Following the initial AbaySteam vision, the next development cycles will introduce:

2, Ethio-Discovery Engine: A dedicated portal for trending regional content and local creator spotlights.

3, Multi-User Auth: JWT-based login with personal "Creator Vaults" for saved media.

4, Real-Time Progress: WebSocket integration to provide a percentage-based visual loading bar for large extractions.
...
Important: The current extraction path is hardcoded for the local user profile; please update the downloads_path in main.py to match your specific Windows username to ensure files are routed to your local Downloads folder correctly.
